/**
 * Jumlah ayat per surat (indeks 0 = surat 1).
 * Sumber: `public/data/quran/*.json` field `ayah_count`.
 */
...
export function getSurahAyahCount(surahId: number): number {
  if (surahId < 1 || surahId > SURAH_AYAH_COUNTS.length) {
    throw new RangeError(`surahId tidak valid: ${surahId}`);
  }
