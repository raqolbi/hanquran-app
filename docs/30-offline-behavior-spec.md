...
| Offline | Belum diunduh | ✅ Baca (jika data SW ada) | ❌ Disabled | ❌ **Sembunyi** (hanya online) |
...
| Cache | Isi | Strategi |
|-------|-----|----------|
| `hanquran-data-v1` | `/data/*` | cache-first |
| `hanquran-audio-v1` | MP3 CDN | cache-first |
| `hanquran-shell-v1` | HTML navigasi + RSC App Router | network-first / stale-while-revalidate |
...
- [ ] Cold start offline: Beranda tampil; surat dengan data ter-cache dapat dibuka (bukan splash → home).
...
| Navigasi surat offline | §6 | ✅ Precache + SW; verifikasi manual perangkat ⏳ |
